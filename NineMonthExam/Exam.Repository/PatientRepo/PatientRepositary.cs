...
namespace Exam.Repository.PatientRepo
{
    public class PatientRepositary : RepositoryBase, SpotInterface
    {
        ...
        public List<Spot> GetAllExcelData(Spot p)
        {
            try
            {
                DynamicParameters ObjParm = new DynamicParameters();
        ...
        public async Task<List<SportEntity>> BindSportByClubId(int id)
        {
            try
            {
                ...
                var GetAppById = Connection.Query<SportEntity>(query, ObjParm, commandType: CommandType.StoredProcedure).AsList();
                return GetAppById;
